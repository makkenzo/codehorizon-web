...
export const InteractiveHoverButton = React.forwardRef<HTMLButtonElement, InteractiveHoverButtonProps>(
    ({ children, className, buttonClassName, icon, ...props }, ref) => {
        return (
            <button
                ref={ref}
                className={cn(
                    'group relative w-auto cursor-pointer overflow-hidden rounded-md border bg-background p-2 px-6 text-center font-semibold h-[41px]',
                    className
                )}
                {...props}
            >
