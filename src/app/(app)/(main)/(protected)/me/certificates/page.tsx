            ...
            {!isLoading && !error && certificates && certificates.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {certificates.map((cert) => (
                        <Card key={cert.id}>
                            <CardHeader>
                                <CardTitle className="text-lg line-clamp-2">{cert.courseTitle}</CardTitle>
                                <CardDescription>Дата получения: {cert.completionDate}</CardDescription>
                                <CardDescription className="text-xs text-muted-foreground/70 pt-1">
